...
output=$(./ovn3-chars nonexisting.txt)
code=$?
...
output=$(./ovn3-chars test.txt)
code=$?
expected_output="Antal tecken $(cat test.txt | tr -cd '\40-\176' |wc -c)"
rm test.txt
...
output=$(./ovn3-chars lorem.txt)
code=$?
expected_output="Antal tecken $(cat lorem.txt | tr -cd '\40-\176' |wc -c)"
